...
const os = require('os'); // 提供了一些基本的系统操作函数 os.cpus() 返回一个对象数组，包含所安装的每个 CPU/内核的信息：型号、速度（单位 MHz）、时间（一个包含 user、nice、sys、idle 和 irq 所使用 CPU/内核毫秒数的对象
const webpack = require('webpack'); // 注意项中
const HappyPack = require('happypack'); // 注意项中
const HtmlWebpackPlugin = require('html-webpack-plugin'); // 注意项中
const MiniCssExtractPlugin = require('mini-css-extract-plugin'); // 注意项中
const happyThreadPool = HappyPack.ThreadPool({ size: os.cpus().length }); // 根据cpu内核数量创建进程池数量
...
module.exports = {
	...
	// webpack.optimize.CommonsChunkPlugin 已删除 使用optimization替换
	optimization: {
		splitChunks: {
